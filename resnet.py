...
class ResNet(nn.Module):
    ...
    def forward(self, x, y=None, l=None):
        ...
        if y is not None and l is not None:            
            nc = self.num_classes
            el = torch.unsqueeze(l[0], 1).repeat(1,nc).float().cuda()
            nx1 = F.one_hot(y[0], num_classes=nc).cuda()
            nx2 = F.one_hot(y[1], num_classes=nc).cuda()
            ny = el * nx1 + (1-el) * nx2
            output = torch.matmul(output, ny.t())
...
def resnet18(dims):
    ...
    """
    return ResNet(BasicBlock, [2, 2, 2, 2], num_classes=dims)
