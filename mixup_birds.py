#
# Perform CIFAIR training and testing with mixup, regmixup, and their
...
def train(model, train_loader, loss_function, mixup_style, alpha, gamma, kappa, epoch):
    ...
    for batch_index, (images, labels) in enumerate(train_loader):
        ...
        if mixup_style == "none":
            ...
            mixed_images, mixed_labels, l = mixup_utils.mixup_data(images, labels, alpha, 224)
            mixed_images = Variable(mixed_images).cuda()
            labels1 = Variable(mixed_labels[0])
            labels2 = Variable(mixed_labels[1])
            ...
            # Mixup Classifier forward.
                ...
                outputs = model(mixed_images, [labels1, labels2], l)
            ...
            # RegMixup = standard output loss + mixup output loss.
                ...
                el = torch.unsqueeze(l[0], 1).repeat(1,outputs2.shape[1]).float()
                nx1 = F.one_hot(mixed_labels[0], num_classes=outputs2.shape[1])
                nx2 = F.one_hot(mixed_labels[1], num_classes=outputs2.shape[1])
                ny = el * nx1 + (1-el) * nx2
                loss2 = loss_function(outputs2, ny.cuda())
            ...
            # Idem with mixup classifiers.
                ...
                loss0 = loss_function(outputs1, labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-w", dest="weight_decay", default=5e-4, type=float)
    ...
    # Get data.
    train_loader, test_loader = utils.get_birds(args.batch_size)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 10 == 0 or epoch == args.epochs -1:
            ...
            print()
            logline = "TEST [%s: e-%d m-%s-%d-%.4f-%.4f n-%s-%d l-%d-%.3f-%d] : %.4f" \
                    %(args.dataset, epoch, args.mixup_style, args.kappa, \
            ...
            if do_log and epoch == args.epochs -1:
                ...
                if args.model_path != "":
                    model_path = args.model_path + "%s_e-%d/m-%s-%d-%.4f-%.4f/n-%s-%d/l-%d-%.5f-%.5f-%d/" \
                            %(args.dataset, epoch, args.mixup_style, args.kappa, \
                    args.alpha, args.gamma, args.network, 0, int(args.use_scheduler), args.learning_rate, args.weight_decay, args.batch_size)
